...
import { GetAllRequestedServiceUseCase, GetRequestedServiceUseCase } from "../../modules/requestedService/useCases/get/RequestedServiceUseCase";


let getAllRequestedServiceUseCase: GetAllRequestedServiceUseCase;
...
let getRequestedServiceUseCase: GetRequestedServiceUseCase;

describe("List all RequestedService", () => {
    beforeEach(() => {
        ...
        getRequestedServiceUseCase = new GetRequestedServiceUseCase(requestedServiceRepositoryInMemory);
        getAllRequestedServiceUseCase = new GetAllRequestedServiceUseCase(requestedServiceRepositoryInMemory);
    });
    it("should be able to list all RequestedService", async () => {
        ...
        const all = await getAllRequestedServiceUseCase.execute();

        expect(all[0]).toHaveProperty("id");
        expect(all.length).toEqual(1);
    })

    it("should be able to findOne id requestedService", async () => {
        const requested: IRequestedServiceDTO = {
            id: "59fde46d-40ad-46ac-a674-a8506c4791f6",
            service_id: "aad78fad-b2a1-4e93-9eaf-2d1ba7a6dce1",
            budget_id: "d72f74da-d1c9-4a79-98c7-7a5986461ae7",
        ...
        const findId = await getRequestedServiceUseCase.execute(requested.id)

        expect(findId.id).toBe("59fde46d-40ad-46ac-a674-a8506c4791f6");
    })
