...
import { UpdateProviderUseCase } from "../../modules/provider/useCases/update/ProviderUseCase";


let createProviderUseCase: CreateProviderUseCase;
let providerRepositoryInMemory: ProviderRepositoryInMemory;
let updateProviderUseCase: UpdateProviderUseCase;
...
describe("Update provider", () => {
    beforeEach(() => {
        ...
        updateProviderUseCase = new UpdateProviderUseCase(providerRepositoryInMemory)
    })

    it("should be able to update one provider", async () => {
        ...
        const findProvider = await providerRepositoryInMemory.findById(provider.id)
        ...
        expect(updateProvider.email).toBe("test@gmail.com");
    })
})